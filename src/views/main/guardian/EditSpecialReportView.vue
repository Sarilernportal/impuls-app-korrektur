  ...
  <div class="mx-auto max-w-3xl px-4 py-4 sm:px-6">
    <div>
      ...
      <error-window
        ...
        title="Fehler"
        message="Ihr Sonderzeitenbericht konnte nicht gefunden werden. Bitte stellen Sie sicher, dass Sie über eine stabile Internetverbindung verfügen."
      ...
      <error-window
        ...
        title="Fehler"
        message="Ihr Sonderzeitenbericht konnte nicht aktualisiert werden. Bitte stellen Sie sicher, dass Sie alle Pflichtfelder ausgefüllt haben und über eine stabile Internetverbindung verfügen."
      ...
      <success-window
        ...
        title="Sonderzeitenbericht aktualisiert"
        message="Ihr Sonderzeitenbericht wurde erfolgreich aktualisiert."
        ...
        <div class="space-y-6 sm:space-y-5">
          ...
          <div>
            ...
            <p class="mt-1 max-w-2xl text-sm text-secondaryText">
              Überarbeiten Sie den Sonderzeitenbericht.
            </p>
          </div>
          <div
            v-if="flagText"
          ...
          <div>
            ...
            <div class="flex flex-col gap-2 sm:border-t sm:border-gray-200">
              ...
              <p class="text-sm text-secondaryText">
                Wählen Sie den Start- und Endzeitpunkt aus.
              </p>
  ...
  setup() {
    ...
    async function getReport() {
      try {
        ...
        if (dailyReport.flagDaySelection === null) {
          allowDaySelection.value = true
        } else {
    ...
    function prefillInput() {
      if (reportFindSuccess.value === 'none') {
        ...
        // check if calendar should allow range selection
        const rangeList = ['employeeSickness', 'vacation']
        if (rangeList.includes(dailyReportData.value.reportActivity)) {
          calendarShowRange.value = true
        } else {
          calendarShowRange.value = false
        }
        ...
        inputData.value.endDate = dailyReportData.value.documentEndDate

        getDefaultSelectedTimeDate.value = {
          date: inputData.value.date,
          endDate: inputData.value.endDate,
    ...
    async function submitReport() {
      if (inputValid.value) {
        try {
          ...
          const resp = await store.dispatch('updateSpecialReport', {
