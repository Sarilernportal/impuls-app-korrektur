  ...
  <div class="min-h-full bg-slate-50 px-4 pb-24 pt-5 sm:px-6 lg:px-8">
    ...
    <div class="flex w-full flex-col gap-5">
      <section class="rounded-xl bg-gradient-to-br from-impuls-blue via-brand-700 to-brand-900 p-5 text-white shadow-soft">
        ...
        <h1 class="mt-1 text-2xl font-bold sm:text-3xl">Kostenträger / Jugendamt hinzufügen</h1>
...
// Component imports
import NewCarrierForm from '@/components/Main/Admin/Carrier/NewCarrierForm.vue'
...
export default {
  ...
  setup(props) {
    ...
    function formSubmitted(userInputs) {
      ...
      addCarrier(mappedInputs)
    }

    // Add the user to the backend
    async function addCarrier(userInputs) {
      isLoading.value = true
      try {
        await store.dispatch('addCarrier', userInputs)
