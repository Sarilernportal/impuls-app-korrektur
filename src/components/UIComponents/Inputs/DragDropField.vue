...
<template>
  ...
  <div class="w-full h-full">
    <!-- upload vfield wrapper -->
    <div
      class="w-full sm:grid sm:grid-cols-1 sm:gap-4 sm:items-start border-2 border-slate-300 border-dashed hover:border-slate-400"
    >
      <div class="py-4 flex items-center justify-center">
        <div
          ...
          class="w-full flex justify-center items-center px-6 rounded-md"
        >
          <div
            class="absolute rounded-full bg-slate-100 h-20 w-20 z-10 transition-opacity duration-500 ease-in-out"
            :class="{
          ...
          <div class="relative z-20 text-center">
            ...
            <p class="mt-5 text-sm text-slate-400">
              Dateien via Drag & Drop in das Feld ziehen
            </p>
    ...
    <div class="shadow overflow-hidden sm:rounded-md mt-5">
      <ul>
        <li
          ...
          v-bind:key="index"
          class="flex justify-between px-2 py-2 sm:px-4 items-center"
        >
          <div
            class="block focus:outline-none focus:bg-slate-50 transition duration-150 ease-in-out"
          >
            <div class="flex items-center justify-between">
              <div class="text-sm leading-5 font-medium text-pink-600 truncate">
                {{ file.name }} ({{ file.size / 1000000.0 }} MB)
              </div>
            </div>
          </div>
          <button
            @click.prevent="deleteEntry(index)"
            class="font-semibold text-center text-red-500 border border-red-400 rounded-full bg-red-100 h-6 w-6"
          >
            X
          </button>
        </li>
    ...
    <div class="w-full flex justify-center mt-4">
      ...
      <standard-button
        class="w-full"
        @button-tapped="uploadButtonTapped"
        title="Hochladen"
...
export default {
  ...
  emits: ['file-updated', 'upload-tapped'],
  setup(_, { emit }) {
    //Ref initialization
    const files = ref([])
    const pageX = ref(0)
    ...
    // Event for dragging over upload area
    function onUploadDragoverEvent(e) {
    ...
    // Event for dropping a file on the upload areay
    function onUploadDropEvent(e) {
    ...
    // Delete one entry in the file list
    function deleteEntry(id) {
      files.value.splice(id, 1)
      emit('file-updated', files.value)
    }

    // Callback, when the user hits the upload button
    function uploadButtonTapped() {
