...
<template>
  ...
  <div class="w-full">
    ...
    <carrier-overview-list-header :listContent="content" />
    <!-- Search Bar -->
    ...
    <carrier-list
      ...
      :carriers="sortedUsers"
      :userType="content"
      :menuItems="menuItems"
      :isLoading="isLoading"
    />
    <!-- display if no other carriers were found -->
...
// Component imports
import CarrierOverviewListHeader from '@/components/Main/Admin/Carrier/CarrierOverviewListHeader.vue'
import PaginationBar from '@/components/Others/PaginationBar.vue'
import CarrierList from '@/components/Main/Admin/Carrier/CarrierList.vue'
...
export default {
  ...
  setup(props) {
    ...
    // Fetch the carrier as well check if we have a token for pagination
    const debouncedFetchCarriers = _.debounce(async function (forward) {
      ...
      try {
        ...
        // Get the response
        const response = await store.dispatch('fetchCarriersOverviewList', payload)
    ...
    function fetchCarriers(forward) {
      debouncedFetchCarriers(forward)
    }
    ...
    function setSearchFieldText() {
      ...
      const exclude = ['Status', 'Bearbeiten', 'Registriert', 'Adresse']
    ...
    const sortedUsers = computed(() => {
      ...
      const sortUsers = carriers.value
