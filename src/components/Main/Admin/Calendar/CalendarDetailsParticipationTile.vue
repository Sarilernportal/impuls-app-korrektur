<template>
  <div>
    ...
    >
      <!-- participation color -->
      <div
        id="participationColor"
        :class="[
          'h-full w-5 rounded-l-full overflow-clip',
          participation.type === 'admin' ? 'bg-red-500' : 'bg-green-500'
        ]"
      ></div>
      ...
      <button
        @click="goToUser"
        :class="[
          'text-slate-700 px-2 bg-slate-100 overflow-clip hover:bg-slate-200 print:border print:border-slate-300',
  ...
  setup(props, { emit }) {
    ...
    const userName = computed(() => {
      try {
        ...
        const participant = props.userList[props.participation.participant]
    ...
    function goToUser() {
      router.push({
        name: 'UserDetails',
        params: { id: props.participation.participant }
      })
    }
