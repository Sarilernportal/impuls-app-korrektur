...
app.post('/createUser', async (req, res, next) => {
  if (!req.body.groupname || !req.body.attributes.email) {
    const err = new Error('user type and email are required');
    err.statusCode = 400;
    return next(err);
  }

  try {
    const response = await createUser(req.body.groupname, req.body.attributes);
...
app.post('/updateUser', async (req, res, next) => {
  if (!req.body.username || !req.body.modify_input) {
    const err = new Error('username, name and modify input are required');
    err.statusCode = 400;
    return next(err);
  }

  try {
    const response = await updateUser(req.body.username, req.body.modify_input);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
